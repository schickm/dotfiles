...
// Nothing but the native port wakes this worker, so once Chrome has stopped it
// (and the host with it) the socket stays gone until someone reloads the
// extension by hand. An alarm is an event that wakes the worker; connect() is
// a no-op while the port is up, so this only ever repairs a dead one.
